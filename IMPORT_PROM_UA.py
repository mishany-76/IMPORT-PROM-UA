...
def synchronize_single_sheet_with_data(gspread_client, sheets_service, source_sheet, target_spreadsheet):
    ...
    try:
        ...
        required_cols_len_target = len(
            updated_target_headers)  # Используем updated_target_headers после ensure_target_structure
        ...
        if update_operations_batch_format:
            ...
            for attempt in range(batch_max_retries):
                try:
                    ...
                    if attempt < batch_max_retries - 1 and (
                        ...
                        logging.warning(
                            f"API Error (вероятно временная) при пакетном обновлении: {e}. Повторная попытка через {batch_retry_delay} сек...")
            ...
            # else: блок for, если не было break (все попытки исчерпаны и не было критической ошибки)
                ...
                logging.error(
                    f"Не удалось выполнить пакетное обновление после {batch_max_retries} попыток для листа '{source_sheet_title}'.")
                ...
                try:
                    ...
                    if data_to_write:  # Вызываем append_rows только если есть данные
                        target_sheet.append_rows(data_to_write,
                                                 value_input_option='USER_ENTERED')  # Используем gspread_client метод
                        logging.info(
...
def ensure_minimum_empty_rows(gspread_client, target_spreadsheet, sheet_name, min_empty_rows):
    ...
    try:
        # 1. Получаем лист (или создаем, если его нет)
        # Поскольку этот код вызывается после основной синхронизации, лист уже должен существовать
        target_sheet = target_spreadsheet.worksheet(sheet_name)

        # 2. Получаем все значения, чтобы определить последнюю заполненную строку
        # Использование get_all_values() ищет последнюю заполненную ячейку (даже если это пустая строка с формулами)
        # Более надежный способ определить *данные* - это найти последнюю строку с непустыми данными.

        # Получаем данные листа (включая пустые строки в конце, если они есть, до конца фактического диапазона)
        all_values = target_sheet.get_all_values()

        # Определяем общее количество строк в листе (фактический размер листа)
        total_rows_in_sheet = len(all_values)

        # Определяем последнюю строку с данными (ищем непустую строку)
        last_data_row_index_0based = -1  # Индекс последней строки с непустыми данными (0-based)
        for i in range(len(all_values) - 1, -1, -1):  # Итерируемся с конца
            # Проверяем, есть ли в строке хоть одно непустое значение
            if any(cell.strip() for cell in all_values[i]):
                last_data_row_index_0based = i
                break

        # Если last_data_row_index_0based = -1, значит, только заголовок (или вообще ничего)
        # Если last_data_row_index_0based = 0, значит, только заголовок (предполагаем, что строка 1 - это заголовок)
        # Последняя строка с данными - это last_data_row_index_0based + 1 (1-based)
        # Последняя строка *с данными или заголовком* (то есть, не пустая)
        last_non_empty_row_1based = last_data_row_index_0based + 1

        # Если есть хотя бы заголовок, то последняя непустая строка - это минимум 1
        if last_data_row_index_0based == -1 and target_sheet.row_count > 0 and len(target_sheet.row_values(1)) > 0:
            # Это случай, когда есть только заголовок (строка 1), но нет данных
            last_non_empty_row_1based = 1
        elif last_data_row_index_0based == -1:
            # Полностью пустой лист (даже без заголовка)
            last_non_empty_row_1based = 0  # Фактически, данных нет

        # Если last_non_empty_row_1based = 0 (лист пуст), то количество пустых строк равно общему количеству строк
        # Если last_non_empty_row_1based > 0, то строки с данными/заголовком занимают last_non_empty_row_1based строк
        # Пустые строки - это строки ПОСЛЕ последней непустой строки, то есть:
        # total_rows_in_sheet - last_non_empty_row_1based

        # Количество пустых строк (после последней строки с данными)
        current_empty_rows = total_rows_in_sheet - last_non_empty_row_1based

        # Для случаев, когда total_rows_in_sheet может быть меньше, чем last_non_empty_row_1based,
        # что может случиться, если get_all_values() вернул не весь лист, а только диапазон с данными.
        # В gspread `row_count` дает максимальное количество строк.
        current_rows_count = target_sheet.row_count  # Фактический размер листа, включая пустые строки

        # Пересчитаем на основе row_count для точности
        ...
        # 3. Сравнение и добавление строк
        if current_empty_rows < min_empty_rows:
            ...
            # gspread.Worksheet.resize(rows=None, cols=None) - увеличивает общее количество строк.
            # Если передать только rows, то увеличится только количество строк.
            max_retries = 5
            retry_delay = 5
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # Увеличиваем количество строк (при этом старые данные сохраняются)
                    ...
                    if attempt < max_retries - 1 and (
                        ...
                        break  # При критической ошибке останавливаем попытки
...
def synchronize_all_sheets():
    ...
    try:
        ...
        # --- НОВАЯ ЛОГИКА: Проверка и добавление пустых строк после завершения синхронизации ---
